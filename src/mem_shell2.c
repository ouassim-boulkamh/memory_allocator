...
void aide()
{
  ...
  fprintf(stderr, "r @ taille:   realloc le bloc precedemment alloue a @ a la nouvelle taille\n");
  ...
  fprintf(stderr, "M         :   afficher la liste de tous les emplacements memoire (libres et occupes)\n");
...
void afficher_zone(void *adresse, size_t taille, int free)
{
  printf("Zone %s, Adresse : %lu, Taille : %lu\n", free ? "libre" : "occupee",
         (unsigned long)adresse - (unsigned long)mem_space_get_addr(), (unsigned long)taille);
}
...
int main(int argc, char **argv)
{
  ...
  while (1)
  {
    ...
    case 'r':
      ...
      ptr = mem_realloc(mem_space_get_addr() + offset, taille);
      if (ptr == NULL)
        printf("Echec du realloc\n");
      else
      {
        allocations[offset - 1] = ptr;
        printf("Memoire reallouee en %d, nouvelle taille: %d\n",
               (int)(ptr - mem_space_get_addr()), taille);
      }
      break;
    case 'i':
